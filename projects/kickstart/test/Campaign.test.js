...
beforeEach(async ()=>{
    ...
    [campaignAddress] = await factory.methods.getDeployedCampaigns().call(); //first element [asdasd]
...
describe('Campaigns', ()=>{
    ...
    it('allow people to contribute money and mark them as approvers',async()=>{
        ...
        const request = await campaign.methods.approvers(accounts[1]).call();
    ...
    it('manager can create a payment request',async()=>{
        await campaign.methods.createRequest(" a description",'100',accounts[1]).send({
            from:accounts[0],
            gas:'1000000'
